...
- **Bahasa layar dibagi dua, dan pembagiannya tegas.** Judul berbahasa Inggris, isi berbahasa
  Indonesia. Keputusan pemilik proyek pada 7 September 2026.
  - Inggris: kelompok menu, nama menu, `modelLabel` dan `pluralModelLabel`, judul kartu
    `Section::make()`, judul relation manager, judul widget, tab dasbor, judul kotak dialog
    (`modalHeading`), seluruh label tombol tindakan, dan `modalSubmitActionLabel`.
  - Indonesia: label field, label kolom tabel, `helperText`, `placeholder`, empty state,
    subjudul halaman, isi pemberitahuan, `modalDescription`, dan seluruh nilai data.
  - Kata bawaan Filament (Buat, Lihat, Ubah, Hapus, Simpan, Batal, Cari) tetap Indonesia
    karena panel memakai locale `id`. Judul halaman karenanya berbunyi campuran seperti
    "Buat Vendor Bill", dan itu memang disengaja.
  - Nama menu mengikuti kode modul yang sudah ada di izin (`assets`, `work_orders`,
    `vendor_bills`), bukan terjemahan bebas, supaya layar dan izin menyebut hal yang sama.
- Kartu formulir tidak boleh ada yang sendirian di barisnya. Skema formulir resource memakai
  dua kolom di layar lebar, jadi kartu yang jumlahnya ganjil menyisakan satu kartu selebar
  setengah layar dengan separuh kanan kosong. Kartu terakhir pada formulir berjumlah ganjil,
  dan satu satunya kartu pada formulir berkartu tunggal, diberi `->columnSpanFull()`.
  Kartu yang tampilnya bersyarat (`->visible()`) selalu diberi `->columnSpanFull()`, karena
  jumlah kartu yang berubah ubah tidak bisa dijaga tetap genap.
