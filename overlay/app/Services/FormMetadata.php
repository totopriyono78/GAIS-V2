...
class FormMetadata
{
    ...
    private function fieldRelasi(string $jalur, string $tabel)
    {
        ...
        return Select::make($jalur)
            ...
            /*
             * Nama argumennya WAJIB $search, dan ini satu satunya tempat di
             * seluruh proyek ini yang tidak boleh memakai bahasa Indonesia.
             *
             * Filament mengisi argumen penutupan berdasarkan namanya, bukan
             * urutannya. Argumen bernama $cari tidak dikenalinya, dan yang
             * terjadi bukan daftar kosong melainkan BindingResolutionException
             * yang menggagalkan seluruh permintaan begitu daftar pilihannya
             * dibuka. Berlaku juga untuk $state, $get, $record, dan $livewire.
             */
            ->getSearchResultsUsing(fn (string $search): array => $model::query()
                ->where(function (Builder $q) use ($kolomCari, $search) {
                ...
                ->all())
            // $value juga nama yang ditentukan Filament, bukan pilihan bebas.
